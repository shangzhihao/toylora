...
def inference():
    ...
    test_loader = load_test_dataset(batch_size=64)
    ...
    pretrained_accuracy, pretrained_class_acc = evaluate_model_accuracy(pretrained_model, test_loader, device)
    print_accuracy_results("Pretrained Model", pretrained_accuracy, pretrained_class_acc)

    # 4. Load LoRA fine-tuned model
    print("\nLoading LoRA fine-tuned model...")
    # Create LoRA model structure
    lora_model = LoRAMLP(pretrained_model, rank=config.lora_rank, alpha=config.lora_alpha)
    ...
    lora_accuracy, lora_class_acc = evaluate_model_accuracy(lora_model, test_loader, device)
    ...
    for digit in range(10):
        ...
        print(f"{digit:<8} {pretrained_acc:<12.2f} {lora_acc:<15.2f} {improvement:<12.2f}")
    
    overall_improvement = lora_accuracy - pretrained_accuracy
    print("-"*70)
    print(f"{'Overall':<8} {pretrained_accuracy:<12.2f} {lora_accuracy:<15.2f} {overall_improvement:<12.2f}")
